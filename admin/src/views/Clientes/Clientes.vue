          ...
          <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              >
                <td class="px-5 py-4 sm:px-6">
                  <span class="font-bold text-gray-800 text-theme-sm dark:text-white/90">
                    #{{ String(cliente.id).padStart(5, '0') }}
                  </span>
                </td>
                <td class="px-5 py-4 sm:px-6">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span class="block text-gray-500 text-theme-xs">{{ cliente.email }}</span>
                    </div>
                  </div>
                </td>
                <td class="px-5 py-4 sm:px-6">
                  <p class="font-semibold text-gray-800 text-theme-sm dark:text-white/90">
                    ${{ Number(cliente.total_pedido || 0).toLocaleString('es-MX', { minimumFractionDigits: 2 }) }}
                  </p>
                </td>
                <td class="px-5 py-4 sm:px-6">
                  <span :class="[
                    'rounded-full px-2.5 py-0.5 text-theme-xs font-medium border',
                    cliente.mp_status === 'approved' || cliente.mp_status === 'in_process'
                    ...
                    {{ statusLabel(cliente.mp_status) }}
                  </span>
                </td>
                <td class="px-5 py-4 sm:px-6"><p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ cliente.created_at }}</p></td>
                <td class="px-5 py-4 sm:px-6">
                  <router-link :to="`/admin/clientes/${cliente.id}`" class="text-brand-500 hover:text-brand-600 font-medium text-theme-sm flex items-center gap-1">
                    Ver detalle
              ...
              <tr v-if="clientes.length === 0">
                <td colspan="8" class="px-5 py-12 text-center text-gray-400 text-theme-sm dark:text-gray-500">Sin clientes registrados aún.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const fetchClientes = async (page = 1) => {
  ...
  try {
    ...
    error.value = 'No se pudieron cargar los clientes. Verifica que el servidor esté activo.'
  } finally {
    loading.value = false
  }
}
